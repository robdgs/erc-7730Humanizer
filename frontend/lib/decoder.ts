...
export async function decodeCalldata(
  ...
  if (calldata.length === 66) {
    throw new Error(
      "This looks like a transaction hash. Please provide the transaction calldata/input data instead. You can find it in the transaction details under 'Input Data'."
    );
  }
  ...
  if (calldata.length < 10) {
    throw new Error(
      "Calldata is too short. It should include at least the function selector (4 bytes)."
    );
  }
  ...
  try {
    ...
    throw new Error(
      ...
      }. Make sure you're using the transaction input data, not the transaction hash.`
    );
  }
