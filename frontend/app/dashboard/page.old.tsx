...
export default function Dashboard() {
  ...
  const loadDashboard = async (walletAddress?: string, web3Provider?: ethers.BrowserProvider) => {
    ...
    try {
      ...
      if (activeProvider && !useMockData) {
        const [portfolioResult, governanceResult, claimableResult] = await Promise.all([
        ...
        const [portfolioData, governanceData, claimableData] = await Promise.all([
          getPortfolio(targetAddress),
          getDaoVotes(targetAddress),
          getClaimable(targetAddress),
        ]);
  ...
  return (
    <main className="min-h-screen" style={{background: '#000000'}}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Header */}
        <div className="mb-8">
          <div className="terminal-dim" style={{fontSize: '0.8rem', marginBottom: '0.5rem'}}>
            <a href="/" style={{textDecoration: 'none', color: '#00ff41'}}>
              &lt; RETURN_TO_MAINFRAME
            </a>
          </div>
          <h1 className="terminal-text terminal-glow" style={{fontSize: '3rem', fontWeight: 'bold', marginBottom: '1rem', letterSpacing: '0.1em'}}>
            &gt; ASSET DASHBOARD
          </h1>
        ...
        {/* Address Input */}
        <div className="terminal-box" style={{padding: '1.5rem', marginBottom: '2rem', border: '2px solid #00ff41'}}>
          ...
          <div style={{display: 'flex', gap: '1rem'}}>
            ...
            <button
              ...
              style={{padding: '0.75rem 2rem', fontSize: '0.9rem', borderColor: '#00ffff', color: '#00ffff'}}
            >
              [CONNECT_WALLET]
            </button>
            <button
              ...
              disabled={loading}
              className="terminal-button"
              style={{padding: '0.75rem 2rem', fontSize: '0.9rem', opacity: loading ? 0.5 : 1, cursor: loading ? 'not-allowed' : 'pointer'}}
            >
            ...
            <button
              ...
              disabled={loading}
              className="terminal-button"
              style={{padding: '0.75rem 2rem', fontSize: '0.9rem', borderColor: '#ffb000', color: '#ffb000', opacity: loading ? 0.5 : 1, cursor: loading ? 'not-allowed' : 'pointer'}}
            >
              [LOAD_MOCK]
            </button>
          </div>
          {connectedAddress && (
            <div className="terminal-cyan" style={{marginTop: '0.5rem', fontSize: '0.8rem'}}>
              &gt;&gt; CONNECTED: {connectedAddress.substring(0, 6)}...{connectedAddress.substring(38)}
              {useMockData && <span className="terminal-amber" style={{marginLeft: '1rem'}}>[MOCK_MODE]</span>}
            </div>
          )}
          {error && (
            <div className="terminal-red" style={{marginTop: '1rem', padding: '0.75rem', border: '1px solid #ff0040', fontSize: '0.85rem'}}>
              &gt;&gt; ERROR: {error}
            </div>
          )}
        ...
        {showLedger && actionFlow && (
          <div style={{
            ...
            <div className="terminal-box" style={{padding: '2rem', maxWidth: '600px', width: '100%', border: '2px solid #00ff41'}}>
              <h2 className="terminal-text terminal-glow" style={{fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1.5rem', textAlign: 'center'}}>
                &gt;&gt; TRANSACTION REVIEW
              </h2>
              <LedgerSimulator
                intent={getIntent()}
        ...
        {portfolioData && (
          <div className="space-y-8">
            ...
            <div className="terminal-box" style={{padding: '1rem', border: '2px solid #00ff41', textAlign: 'center'}}>
              <div className="terminal-text terminal-glow" style={{fontSize: '1.5rem', letterSpacing: '0.2em'}}>
                &gt;&gt;&gt; ACCESS GRANTED &lt;&lt;&lt;
              </div>
              <div className="terminal-dim ascii-spinner" style={{marginTop: '0.5rem', fontSize: '0.8rem'}}>
            ...
            {/* Portfolio Overview */}
            <div>
              <h2 className="terminal-text" style={{fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem', letterSpacing: '0.1em'}}>
                &gt; PORTFOLIO_OVERVIEW
              </h2>
              <PortfolioCard
            ...
            {claimableData && claimableData.claimableTokens.length > 0 && (
              <div>
                ...
                <div className="terminal-box" style={{marginBottom: '1rem', padding: '1rem', border: '1px solid #00ff41'}}>
                  <div className="terminal-dim" style={{fontSize: '0.8rem', marginBottom: '0.25rem'}}>
                    &gt; TOTAL CLAIMABLE
                  </div>
                  <div className="terminal-cyan terminal-glow" style={{fontSize: '2rem', fontWeight: 'bold'}}>
            ...
            {/* Event Stream Footer */}
            <div className="terminal-box" style={{padding: '1rem', border: '1px solid #004d1a'}}>
              <div className="terminal-dim" style={{fontSize: '0.75rem', letterSpacing: '0.1em'}}>
        ...
        {!portfolioData && !loading && (
          <div className="terminal-box" style={{padding: '3rem', textAlign: 'center', border: '2px solid #004d1a'}}>
            <div className="terminal-amber terminal-glow" style={{fontSize: '4rem', marginBottom: '1rem'}}>
              [ ! ]
            </div>
            <h3 className="terminal-text" style={{fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '0.5rem'}}>
              &gt; AWAITING CONNECTION
            </h3>
            <p className="terminal-dim" style={{fontSize: '0.9rem'}}>
              ENTER WALLET ADDRESS TO ACCESS DASHBOARD
            </p>
            <div className="terminal-dim" style={{marginTop: '2rem', fontSize: '0.75rem', letterSpacing: '0.05em'}}>
